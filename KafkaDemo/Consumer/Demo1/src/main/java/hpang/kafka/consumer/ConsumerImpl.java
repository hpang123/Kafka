...
public class ConsumerImpl implements Consumer {
	...
	public void start() {
		 ...
		 * messages.
		 */
		consumerProperties.put("group.id", "demogroup1");
		 ...
		 * If you set enable.auto.commit to true, then you might also want to
		...
		try {
			...
			try {
				/*
				 * if we know that this is the last commit before we close the consumer, 
				 * or before a rebalance, we want to make extra sure that the commit succeeds.
				 * So use commitSync() here
				 * commit the latest offset returned by poll() and return once
				 * the offset is committed.
				 * commitSync() will retry the commit 
				 * until it either succeeds or encounters a nonretriable failure,
				 */
				consumer.commitSync();
